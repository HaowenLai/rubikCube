...
#else
const char bus_info[][25] = {"usb-3f980000.usb-1.5", "usb-3f980000.usb-1.3"}; //down, up
#endif

//up camera perspective quad points ---  up, right, back
const vector<vector<Point2f>> quadPtSrcUp{
    vector<Point2f>{Point2f(564, 242), Point2f(380, 180), Point2f(335, 45), Point2f(490, 111)},
    vector<Point2f>{Point2f(317, 45), Point2f(376, 182), Point2f(250, 350), Point2f(220, 200)},
    vector<Point2f>{Point2f(378, 198), Point2f(570, 258), Point2f(450, 408), Point2f(260, 372)}};
...
const vector<vector<Point2f>> quadPtSrcDn{
    ...
    vector<Point2f>{Point2f(572, 333), Point2f(423, 457), Point2f(209, 395), Point2f(351, 233)},
    vector<Point2f>{Point2f(334, 239), Point2f(208, 381), Point2f(217, 207), Point2f(325, 57)}};
...
{
    const int dev_num = 2;
    const char dev_list[][20] = {"/dev/video0", "/dev/video1"};
    char bus_info[dev_num][25];

    for (int i = 0; i < dev_num; ++i)
    {
        int fd = device_open(dev_list[i]);
        device_busInfo(bus_info[i], fd);
        printf("video%d: %s\n", i, bus_info[i]);
        device_close(fd);
    }
}
...
void inferUnknownClr(char colorLetters[])
{
    ...
    const int rest2ClrConterIdx[6][2]{{11, 18}, {0, 38}, {27, 8}, {26, 47}, {51, 15}, {42, 35}};
                    ...
                    std::string modelPath)
{
    ...
    //aquire training samples
    cout << "press 'c' to confirm the result and start\n";
                    ...
                    std::string modelPath)
{
    ...
    //check whether model files exist
    static string modelPathUp = modelPath + "colorParamsUp.xml";
    static string modelPathDn = modelPath + "colorParamsDn.xml";
    if (access(modelPathUp.c_str(), F_OK) || access(modelPathDn.c_str(), F_OK))
    {
        printf("color model file(s) do not exist!\n");
        exit(-1);
    ...
    //load svm model and init cube.
    static Ptr<ml::SVM> modelDn = ml::SVM::load(modelPathDn);
    static Ptr<ml::SVM> modelUp = ml::SVM::load(modelPathUp);
    ...
    //aquire training samples
    vector<float> blkResult;
    cout << "recognizing.. please wait." << endl;
